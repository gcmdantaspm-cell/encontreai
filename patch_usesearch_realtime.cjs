...
const newUseSearch = `function useSearch() {
  ...
  useEffect(() => {
    const unsubUsers = onSnapshot(query(collection(db, 'users'), where('role', '==', 'professional')), (usersSnap) => {
      onSnapshot(collection(db, 'services'), (servicesSnap) => {
        ...
        // Also we need to attach services that belong to mock users!
        const mergedMock = PROFESSIONALS.map(mockPro => {
            const dbSvc = allDbServices.filter(s => s.professionalId === mockPro.id);
            return {
                ...mockPro,
                services: [...(mockPro.services || []), ...dbSvc]
            };
        });

        setPros([...mergedMock, ...dbPros]);
      });
    });
...
console.log('Patched useSearch realtime');
