...
const regex = /const send = async \(receiverId: string, text: string\) => \{[\s\S]*?return \{ msgs, send \};\n\}/m;
...
if (code.match(regex)) {
  ...
  console.log("Patched wildcard successfully");
} else {
  console.log("Wildcard didn't match");
}
